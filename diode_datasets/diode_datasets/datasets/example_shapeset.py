...
from torch_diode.types.matmul_types import MMShape, OperationShapeSet, ShapeSet
...
if __name__ == "__main__":
    ...
    total_shapes = sum(
        len(shape_set.shapes)
        for shape_set in operation_shapeset_from_csv.operations.values()
    )
    operation_names = operation_shapeset_from_csv.get_operation_names()

    print(
        f"Created OperationShapeSet with {len(operation_names)} operations and {total_shapes} total shapes from {shapes_csv_path}"
    )
    ...
    save_operation_shapeset_to_json(
        operation_shapeset_from_csv, operation_shapeset_json_path
    )
