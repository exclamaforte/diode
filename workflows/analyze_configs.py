...
def analyze_max_autotune_solution(solution_path: str) -> List[TritonGEMMConfig]:
    ...
    print(
        f"Max-autotune solution loaded with {len(max_autotune_solution.config)} configs"
    )
    return max_autotune_solution.config


def find_closest_configs(
    dataset_configs: Set[TritonGEMMConfig], max_autotune_configs: List[TritonGEMMConfig]
) -> Dict[str, any]:
...
def generate_better_max_autotune_solution(
    ...
    print(
        f"Creating max-autotune solution with {len(available_configs)} configs from dataset"
    )

    # Create the solution
    solution_dict = {"config": [], "version": 1}
...
def main():
    ...
    validation_dataset_path = (
        "/home/gabeferns/diode/workflows/data/validation/my_validation_86.msgpack"
    )
    max_autotune_solution_path = "/home/gabeferns/diode/workflows/max_autotune.json"
    improved_solution_path = (
        "/home/gabeferns/diode/workflows/data/max_autotune_improved.json"
    )
    ...
    try:
        ...
        closest_matches = find_closest_configs(
            dataset_analysis["all_configs"], max_autotune_configs
        )
        ...
        if exact_matches == 0:
            ...
            print(
                "   The current max-autotune solution contains configs that don't exist in the dataset."
            )
            print(
                f"   Use the improved solution ({improved_solution_path}) for meaningful validation."
            )
