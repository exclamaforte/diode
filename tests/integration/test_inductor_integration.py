...
from diode.model.matmul_timing_model import MatmulTimingModel
...
class TestDiodeInductorChoices(unittest.TestCase):
    ...
    def test_initialization_with_model_path(self):
        ...
        self.assertTrue(choices.enable_fallback)
    ...
    def test_finalize_mm_configs_no_model(self):
        """Test _finalize_mm_configs without model."""
    ...
    def test_finalize_mm_configs_with_model(self):
        """Test _finalize_mm_configs with model."""
        choices = DiodeInductorChoices(
            model_path=self.temp_model_path,
            device=self.device,
            top_k_configs=2,
    ...
    def test_finalize_mm_configs_empty_choices(self):
        """Test _finalize_mm_configs with empty choices."""
        choices = DiodeInductorChoices(
            model_path=self.temp_model_path, device=self.device
        )
        ...
        self.assertEqual(choices.stats["no_choices"], 1)
...
class TestFactoryFunctions(unittest.TestCase):
    ...
    def test_install_diode_choices(self, mock_v):
        ...
        install_diode_choices(device="cpu", top_k_configs=3)

        # Should call set_choices_handler with DiodeInductorChoices instance
        mock_v.set_choices_handler.assert_called_once()
        args = mock_v.set_choices_handler.call_args[0]
        self.assertIsInstance(args[0], DiodeInductorChoices)
    ...
    def test_install_diode_choices_import_error(self):
        ...
        with mock.patch.dict("sys.modules", {"torch._inductor.virtualized": None}):
            with mock.patch(
                "diode.integration.inductor_integration.torch._inductor.virtualized.V",
