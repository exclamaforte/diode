...
class TestDiodeInductorChoices:
    ...
    def test_load_model_success(self):
        ...
        with patch('torch_diode.integration.inductor_integration.ModelWrapper') as mock_wrapper_class:
            with patch.object(DiodeInductorChoices, '_find_default_model', return_value=None):
                mock_wrapper = Mock()
    ...
    def test_finalize_template_configs_success(self):
        ...
        with patch('torch_diode.integration.inductor_integration.convert_and_run_inference_pipeline') as mock_pipeline:
            with patch.object(choices, '_create_unified_predictor') as mock_create_predictor:
                mock_predictor = Mock()
    ...
    def test_finalize_template_configs_no_model_no_fallback(self):
        ...
        with pytest.raises(RuntimeError, match="No model available and fallback is disabled"):
            choices._finalize_template_configs(
                template_choices=template_choices,
                kernel_inputs=Mock(),
                templates=[Mock()],
...
class TestCreateDiodeChoices:
    ...
    def test_create_diode_choices_with_path(self):
        ...
        with patch.object(DiodeInductorChoices, '__init__', return_value=None) as mock_init:
...
class TestExhaustiveConfigGeneration:
    ...
    def test_generate_exhaustive_configs_for_templates_success(self):
        ...
        with patch('torch._inductor.template_heuristics.get_template_heuristic') as mock_get_heuristic:
            with patch('torch._inductor.kernel_template_choice.make_ktc_generator') as mock_make_ktc:
                with patch('torch._inductor.kernel_inputs.MMKernelInputs') as mock_mm_kernel_inputs:
                    ...
                    mock_heuristic = Mock()
                    mock_heuristic.get_exhaustive_mm_configs.return_value = lambda m, n, k, dtype_size, op_name: [
                        Mock(BLOCK_M=64, BLOCK_N=64, BLOCK_K=32, num_stages=2, num_warps=4),
                        Mock(BLOCK_M=128, BLOCK_N=128, BLOCK_K=32, num_stages=3, num_warps=8),
                        # Add more mock configs to simulate exhaustive generation
                    ] * 500  # This should give us ~1000 configs
                    ...
                    mock_ktc_choices = [Mock(template=mock_template, config=Mock()) for _ in range(1000)]
                    ...
                    assert len(result) >= 900, f"Expected ~1000 configs, got {len(result)}"
                    assert len(result) <= 1100, f"Expected ~1000 configs, got {len(result)}"
                    ...
                    mock_heuristic.adjust_kernel_inputs.assert_called_once_with(mock_kernel_inputs, "mm")
    ...
    def test_finalize_template_configs_non_mm_operation(self):
        ...
        with patch.object(DiodeInductorChoices.__bases__[0], '_finalize_template_configs', return_value=mock_super_result) as mock_super:
            template_choices = {"template1": iter([Mock()])}
            kernel_inputs = Mock()
            templates = [Mock()]
    ...
    def test_finalize_template_configs_base_and_nonbase_separation(self):
        ...
        # Mock the methods
        with patch.object(choices, '_generate_exhaustive_configs_for_templates', return_value=[Mock(), Mock()]) as mock_gen:
            with patch.object(choices, '_run_model_inference_on_configs', return_value=[Mock()]) as mock_inference:
                with patch.object(DiodeInductorChoices.__bases__[0], '_finalize_template_configs', return_value=[Mock(), Mock()]) as mock_super:
                    ...
                    assert len(result) >= 2  # At least one from model + two from superclass
...
class TestInstallDiodeChoices:
    ...
    def test_install_with_model_path(self, mock_v):
        ...
        with patch('torch_diode.integration.inductor_integration.create_diode_choices') as mock_create:
            mock_choices = Mock()
