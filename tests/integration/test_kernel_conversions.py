...
class TestMMShapeExtraction:
    ...
    def test_extract_mmshape_basic(self):
        ...
        with patch('torch_diode.integration.kernel_conversions.isinstance') as mock_isinstance:
...
class TestFeatureCreationAndInference:
    ...
    def test_create_features_and_run_inference(self):
        ...
        with patch('torch_diode.integration.kernel_conversions.create_features_from_mmshape_and_configs') as mock_create_features:
            mock_problem_features = torch.randn(2, 7)
            mock_config_features = torch.randn(2, 5)
            mock_create_features.return_value = (mock_problem_features, mock_config_features)
    ...
    def test_create_features_with_model_wrapper_interface(self):
        ...
        mmshape = MMShape(
            B=1, M=64, M_dtype=torch.float32, N=128, K=32, K_dtype=torch.float32,
            out_dtype=torch.float32, out_size=(1, 64, 128), out_stride=(128, 128, 1),
        )
        configs = [TritonGEMMConfig(name="test", grid=1, block_m=64, block_n=64, block_k=32, group_m=8, num_stages=4, num_warps=4)]
        ...
        with patch('torch_diode.integration.kernel_conversions.create_features_from_mmshape_and_configs') as mock_create_features:
            ...
            predictions = create_features_and_run_inference(mmshape, configs, mock_model)
