...
class TestTritonGEMMConfigParse:
    ...
    def test_parse_missing_required_fields(self):
        ...
        with pytest.raises(KeyError, match="Missing required field: block_k"):
            TritonGEMMConfig.parse('{"name": "test", "grid": 1024, "block_m": 64, "block_n": 128}')
        
        # Missing group_m
        with pytest.raises(KeyError, match="Missing required field: group_m"):
            TritonGEMMConfig.parse('{"name": "test", "grid": 1024, "block_m": 64, "block_n": 128, "block_k": 32}')
        
        # Missing num_stages
        with pytest.raises(KeyError, match="Missing required field: num_stages"):
            TritonGEMMConfig.parse('{"name": "test", "grid": 1024, "block_m": 64, "block_n": 128, "block_k": 32, "group_m": 8}')
        
        # Missing num_warps
        with pytest.raises(KeyError, match="Missing required field: num_warps"):
            TritonGEMMConfig.parse('{"name": "test", "grid": 1024, "block_m": 64, "block_n": 128, "block_k": 32, "group_m": 8, "num_stages": 3}')
...
class TestMMShapeParse:
    ...
    def test_parse_missing_required_fields(self):
        ...
        with pytest.raises(KeyError, match="Missing required field: K_dtype"):
            MMShape.parse('{"B": 2, "M": 128, "N": 256, "K": 512, "M_dtype": "float32"}')
        
        # Missing out_dtype
        with pytest.raises(KeyError, match="Missing required field: out_dtype"):
            MMShape.parse('{"B": 2, "M": 128, "N": 256, "K": 512, "M_dtype": "float32", "K_dtype": "float32"}')
        
        # Missing out_size
        with pytest.raises(KeyError, match="Missing required field: out_size"):
            MMShape.parse('{"B": 2, "M": 128, "N": 256, "K": 512, "M_dtype": "float32", "K_dtype": "float32", "out_dtype": "float32"}')
        
        # Missing out_stride
        with pytest.raises(KeyError, match="Missing required field: out_stride"):
            MMShape.parse('{"B": 2, "M": 128, "N": 256, "K": 512, "M_dtype": "float32", "K_dtype": "float32", "out_dtype": "float32", "out_size": [2, 128, 256]}')
...
class TestShapeSetAndOperationShapeSet:
    ...
    def test_operation_shapeset_deserialization_failure(self):
        ...
        result = OperationShapeSet.deserialize('{"operations": {"mm": {"invalid": "data"}}}')
