...
class TestMatmulDatasetCollector(unittest.TestCase):
    ...
    def test_feedback_handler_mm(self):
        ...
        mock_choice = MagicMock(
            spec=torch._inductor.select_algorithm.TritonTemplateCaller
        )
    ...
    def test_estimate_memory_usage(self):
        ...
        memory_usage = self.collector._estimate_memory_usage(
            size, torch.float32, "mm", "cuda"
        )
    ...
    def test_check_memory_feasible_cuda(
        self, mock_memory_allocated, mock_device_properties, mock_cuda_available
    ):
        ...
        size = (128, 128, 128)
        is_feasible = self.collector._check_memory_feasible(
            size, torch.float32, "mm", "cuda"
        )
    ...
    @patch("torch.randn")
    @patch("torch.compile")
    def test_run_matrix_multiplication_memory_skip(
        self,
        mock_compile,
        mock_randn,
        ...
        self.collector._run_matrix_multiplication(
            size, torch.float32, "mm", "cuda", "max-autotune"
        )
    ...
    @patch("torch.randn")
    @patch("torch.compile")
    def test_run_matrix_multiplication_memory_proceed(
        self,
        mock_compile,
        mock_randn,
        mock_memory_allocated,
    ...
    def test_memory_check_logging(self, mock_logger):
        ...
        with patch("torch.cuda.get_device_properties") as mock_props, patch(
            ...
            self.assertIn(
                "Skipping mm with size (100000, 100000, 100000)", warning_call
            )
