...
class TestValidateMaxAutotune:
    ...
    def test_successful_directory_loading(self, mock_model_class, mock_create_dataloaders):
        ...
        mock_dataloader.dataset.dataset.timing_dataset.problem_features = torch.zeros((0, 4))
    ...
    @patch('torch_diode.model.model_utils.MatmulDataset')
    @patch('torch_diode.model.model_utils.DeepMatmulTimingModel')
    def test_msgpack_dataset_loading(self, mock_model_class, mock_dataset_class, mock_create_dataloaders):
    ...
    def test_per_op_max_autotune_format(self):
        ...
        with patch('torch_diode.model.model_utils.create_dataloaders') as mock_create_dataloaders:
            mock_dataloader = Mock()
            mock_dataloader.dataset.dataset.problem_feature_dim = 4
            mock_dataloader.dataset.dataset.config_feature_dim = 6
            mock_dataloader.dataset.dataset.timing_dataset = Mock()
            ...
            with patch('torch_diode.model.model_utils.DeepMatmulTimingModel'):
                with patch('torch_diode.model.model_utils.MatmulDataset.deserialize') as mock_deserialize:
    ...
    def test_base_model_loading(self):
        ...
        with patch('torch_diode.model.model_utils.create_dataloaders') as mock_create_dataloaders:
            ...
            with patch('torch_diode.model.model_utils.MatmulTimingModel') as mock_base_model:
                with patch('torch_diode.model.model_utils.MatmulDataset.deserialize') as mock_deserialize:
                    mock_deserialize.return_value = Mock()
                    mock_model = Mock()
    ...
    def test_direct_state_dict_loading(self):
        ...
        with patch('torch_diode.model.model_utils.create_dataloaders') as mock_create_dataloaders:
            ...
            with patch('torch_diode.model.model_utils.DeepMatmulTimingModel') as mock_deep_model:
                with patch('torch_diode.model.model_utils.MatmulDataset.deserialize') as mock_deserialize:
                    mock_deserialize.return_value = Mock()
                    mock_model = Mock()
