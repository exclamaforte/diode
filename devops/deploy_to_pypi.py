    ...
    if environment == 'test':
        ...
        if not (os.getenv('TESTPYPI_USERNAME') and os.getenv('TESTPYPI_PASSWORD')):
            ...
            if not pypirc_path.exists():
                print_warning("No TestPyPI credentials found in environment variables or ~/.pypirc")
                print("Set TESTPYPI_USERNAME and TESTPYPI_PASSWORD environment variables")
        ...
        if not (os.getenv('PYPI_USERNAME') and os.getenv('PYPI_PASSWORD')):
            pypirc_path = Path.home() / '.pypirc'
            if not pypirc_path.exists():
                print_warning("No PyPI credentials found in environment variables or ~/.pypirc")
...
def main():
    ...
    parser.add_argument(
        '--build-only',
        action='store_true',
        help='Only build packages, do not upload'
    )
    ...
    if args.build_only:
        ...
        if args.environment == 'test':
            ...
            print("  pip install --index-url https://test.pypi.org/simple/ torch-diode-lib")
        else:
